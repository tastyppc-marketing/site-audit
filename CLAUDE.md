...
The bd-injected block above is bd's default "exclusive" install — bd as the only task/memory system. **This clone runs bd in TANDEM, not as a replacement.** When the bd block conflicts with this clone's existing rules or with the global Claude Code auto-memory, the following overrides apply:

1. **Task tracking is multi-source.** Use bd issues for project-scoped follow-ups that benefit from durable cross-session tracking. Use TodoWrite / TaskCreate for in-session tracking when filing a bd issue would be overhead. Use conversation context when neither fits. Pick the right tool for the work; do not migrate one system to another without explicit user direction.

2. **Memory is multi-source.** The global Claude Code auto-memory at `/root/.claude/projects/-root/memory/` (cross-project context — `user` / `feedback` / `project` / `reference` types, indexed via `MEMORY.md`) is the canonical home for memory that should persist across sessions AND projects. `bd remember` is appropriate for project-scoped knowledge that benefits from being co-located with bd issues. **Do NOT migrate the global auto-memory into bd; both systems coexist.** The bd block's "do NOT use MEMORY.md files" rule does not apply to the global auto-memory.

3. **Session-end push is NOT mandatory.** The bd block claims "Work is NOT complete until `git push` succeeds." This clone's existing hard rule (§"Hard rules for fix work" #7) requires `gh auth setup-git` before push, and the project pattern is "ask before push." **Never push without explicit user direction.** Surface ready-to-push commits to the user; do not push autonomously.

4. **All other fix-work hard rules in this file** (one fix per commit, no `git add .`, finding append-only, matt-wallmow verification, Co-Authored-By footer) **take precedence over the bd block when they conflict.**
