...
(function () {
  ...
  function renderOrphanPages(data) {
    ...
    var rows = orphans.map(function (item) {
      ...
      return '<tr class="no-break">' +
        '<td><div class="font-medium text-slate-800 break-all" title="' + _esc(item.url) + '">' + _esc(item.url) + '</div></td>' +
  ...
  function renderHubSpokeClusters(data) {
    ...
    var cards = clusters.map(function (cluster) {
      ...
      return '<div class="bg-white border border-slate-200 rounded-xl p-6 no-break">' +
        '<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">' +
          '<div class="min-w-0">' +
            '<div class="text-xs uppercase tracking-[0.16em] text-slate-400">Hub URL</div>' +
            '<div class="mt-1 text-base font-semibold text-slate-800 break-all" title="' + _esc(cluster.hubUrl) + '">' + _esc(cluster.hubUrl) + '</div>' +
          '</div>' +
