 ...
 *
 * Optimizations:
 *   - Progressive rendering: sections are rendered in requestAnimationFrame
 *     batches so the first sections paint while later ones are still building
 *   - Table overflow detection: adds scrollable-hint class and hides it after
 *     user scrolls, plus attaches scroll listener for sticky-column tables
 *   - Deferred chart initialization via the lazy-render system in charts.js
 */
(function () {
  'use strict';
  ...
  // -------------------------------------------------------------------------
  // Table overflow detection: add scroll hints on mobile
  // -------------------------------------------------------------------------
  function setupTableScrollHints(container) {
    if (!container) return;
    var wraps = container.querySelectorAll('.report-table-wrap');
    wraps.forEach(function (wrap) {
      // Check if the table overflows its container
      if (wrap.scrollWidth > wrap.clientWidth + 4) {
        wrap.classList.add('scrollable-hint');

        // Remove the hint label after user scrolls
  ...
  function renderPageSpeed(data) {
    ...
    if (comparison.length) {
      html += '<div class="chart-container mb-8">' +
        '<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">' +
          ...
          '</div>' +
        '</div>' +
        '<div style="height:clamp(200px, 40vw, 320px)"><canvas id="pagespeed-comparison-chart"></canvas></div>' +
      '</div>';
    }
    ...
    lighthouseResults.forEach(function (result) {
      ...
      // Use responsive grid: 2 cols on sm, 3 on lg, 6 on xl
      html += '<article class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-sm">' +
        '<div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">' +
          '<div class="min-w-0">' +
            ...
            '</div>' +
            '<div class="text-sm sm:text-base font-bold text-slate-800 break-all">' + esc(result.url || 'Untitled URL') + '</div>' +
          '</div>' +
        '</div>' +
        '<div class="grid gap-3 grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 mt-5">' +
    ...
    setupTableScrollHints(container);
  }
      ...
      '<div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)] mb-8">' +
        '<div class="chart-container">' +
          '<div class="flex items-center justify-between gap-3 mb-4">' +
            ...
            '</div>' +
          '</div>' +
          '<div style="height:clamp(180px, 35vw, 288px)"><canvas id="schema-coverage-chart"></canvas></div>' +
        '</div>' +
  ...
  function renderMetaTagAudit(data) {
    ...
    container.innerHTML =
      '<div class="grid gap-4 grid-cols-2 sm:grid-cols-2 xl:grid-cols-4 mb-8">' +
  ...
  function renderSiteStructure(data) {
    ...
    container.innerHTML =
      '<div class="grid gap-4 grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 mb-8">' +
  ...
  // -------------------------------------------------------------------------
  // Progressive section rendering: use requestAnimationFrame to yield to the
  // browser between sections so the first sections paint immediately.
  // -------------------------------------------------------------------------
  function renderProgressively(data, renderers, index) {
    if (index >= renderers.length) return;

    renderers[index](data);

    if (index + 1 < renderers.length) {
      requestAnimationFrame(function () {
        renderProgressively(data, renderers, index + 1);
      });
    }
  }
  ...
  window.TPPC.pages.technical = {
    init: function (data) {
      // Render sections progressively: each section gets its own animation frame
      // so earlier sections paint while later ones are being computed.
      var renderers = [
        this.renderCoreWebVitals,
        this.renderPageSpeed,
        this.renderSchemaAudit,
        this.renderMetaTagAudit,
        this.renderCrawlIssues,
        this.renderSiteStructure
      ];

      renderProgressively(data, renderers, 0);
    },
