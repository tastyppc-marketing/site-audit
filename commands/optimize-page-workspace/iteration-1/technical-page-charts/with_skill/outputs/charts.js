...
(function () {
  ...
  var DEFAULT_FONT = { family: "'Inter', ui-sans-serif, system-ui, sans-serif", size: isMobile ? 10 : 12 };
  ...
  function _baseOptions(overrides) {
    return Object.assign({
      ...
      plugins: {
        legend: {
          labels: { font: DEFAULT_FONT, usePointStyle: true, padding: isMobile ? 10 : 16 }
        }
      }
    }, overrides || {});
  }
  ...
  function _gaugeHTML(label, val, t) {
    ...
    var colorLabel = color === 'green' ? 'Good' : color === 'orange' ? 'Needs Improvement' : 'Poor';
    ...
    return '<div class="mb-4 no-break">' +
      ...
      '</div>' +
      '<div class="cwv-gauge" role="meter" aria-label="' + label + ': ' + val + t.unit + ' (' + colorLabel + ')" aria-valuenow="' + val + '" aria-valuemin="0" aria-valuemax="' + t.max + '">' +
        '<div class="cwv-gauge-fill ' + color + '" style="width:' + pct + '%"></div>' +
        '<div class="cwv-gauge-marker" style="left:' + goodPct + '%" aria-hidden="true"></div>' +
        '<div class="cwv-gauge-marker" style="left:' + poorPct + '%" aria-hidden="true"></div>' +
      '</div>' +
  ...
  function renderCWVGauges(containerId, cwvData) {
    ...
    if (cwvData.mobile) {
      html += '<div class="bg-white border border-slate-200 rounded-xl p-4 sm:p-6">';
      html += '<h3 class="font-bold text-slate-800 mb-4 flex items-center gap-2">';
      html += '<svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">';
