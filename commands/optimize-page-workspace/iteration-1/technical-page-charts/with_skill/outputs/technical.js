...
(function () {
  ...
  function emptyState(title, message) {
    return '<div class="empty-state" role="status" aria-label="' + esc(title) + '">' +
  ...
  function renderSiteStructure(data) {
    ...
    container.innerHTML =
      '<div class="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 mb-8">' +
  ...
  /**
   * Detect scrollable table wrappers and add .is-scrollable class
   * for the CSS scroll-hint fade effect.
   */
  function detectScrollableTables() {
    var wraps = document.querySelectorAll('.report-table-wrap');
    for (var i = 0; i < wraps.length; i++) {
      ...
      // Also add tabindex for keyboard scroll and aria role
  ...
  window.TPPC.pages.technical = {
    init: function (data) {
      ...
      // After all content is rendered, detect scrollable tables
      detectScrollableTables();
      // Re-check on resize (debounced)
      var resizeTimer;
      window.addEventListener('resize', function () {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(detectScrollableTables, 250);
      });
    },
