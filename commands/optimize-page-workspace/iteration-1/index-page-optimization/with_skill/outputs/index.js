      ...
      _setHTML('#stats-grid', stats.map(function (stat) {
        return '' +
          '<div class="stat-card ' + _statSeverityClass(stat.severity) + ' animate-in no-break">' +
            '<div class="stat-value" aria-label="' + _text(stat.label) + ': ' + _text(stat.value) + '">' + _text(stat.value) + '</div>' +
      ...
      _setHTML('#issues-list', issues.map(function (item, index) {
        ...
        return '' +
          '<div class="issue-card animate-in no-break">' +
            '<div class="issue-number" aria-hidden="true">' + _text(index + 1) + '</div>' +
            '<div class="flex-1 min-w-0">' +
              '<div class="flex flex-wrap items-center gap-2 mb-1">' +
                '<span class="font-bold text-slate-800 break-words">' + _text(item.issue) + '</span>' +
                '<span class="severity-badge ' + severityClass(item.impact) + '">' + _text(item.impact) + '</span>' +
                effort +
              '</div>' +
              '<p class="text-sm text-slate-500 leading-relaxed break-words">' + _text(item.detail) + '</p>' +
            '</div>' +
          '</div>';
      }).join(''));
    },
      ...
      _setHTML('#quickwins-list', wins.map(function (item) {
        ...
        return '' +
          '<div class="quickwin-item no-break">' +
            '<div class="quickwin-check" aria-hidden="true">' +
            ...
            '</div>' +
            '<div class="flex-1 min-w-0">' +
              '<span class="text-sm font-medium text-slate-700 break-words">' + _text(item.action) + '</span>' +
            '</div>' +
            impact +
          '</div>';
      }).join(''));
    ...
    renderNextSteps: function (data) {
      ...
      _setHTML('#nextsteps-list', nextSteps.map(function (item, index) {
        var sub = item && item.sub
          ? '<p class="text-sm text-slate-500 mt-0.5 break-words">' + _text(item.sub) + '</p>'
          : '';

        return '' +
          '<div class="nextstep-card animate-in no-break">' +
            '<div class="nextstep-number" aria-hidden="true">' + _text(index + 1) + '</div>' +
            '<div class="min-w-0">' +
              '<div class="font-semibold text-slate-800 break-words">' + _text(item.text) + '</div>' +
              sub +
            '</div>' +
          '</div>';
      }).join(''));
