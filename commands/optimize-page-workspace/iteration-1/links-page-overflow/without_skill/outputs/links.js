...
(function () {
  ...
  function renderLinkOverview(data) {
    ...
    var html = '';
    html += '<div class="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">';
    ...
    html += '<div class="grid sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-8">';
    ...
    if (summary.domain || issues.length || recommendations.length) {
      html += '<div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-8">';
  ...
  function renderOrphanPages(data) {
    ...
    /* Mobile: render as stacked cards instead of a table */
    var isMobile = window.innerWidth < 640;

    var tableRows = orphans.map(function (item) {
      ...
      return '<tr class="no-break">' +
        '<td><div class="url-cell font-medium text-slate-800" title="' + _esc(item.url) + '">' + _esc(_shortUrl(item.url, 60)) + '</div></td>' +
    ...
    var mobileCards = orphans.map(function (item) {
      var sitemapBadge = item.isInSitemap
        ? '<span class="severity-badge info">In sitemap</span>'
        : '<span class="severity-badge medium">Not in sitemap</span>';

      return '<div class="bg-white border border-slate-200 rounded-xl p-4 no-break">' +
        '<div class="text-xs uppercase tracking-widest text-slate-400 mb-1">URL</div>' +
        '<div class="text-sm font-medium text-slate-800 break-all mb-3" title="' + _esc(item.url) + '">' + _esc(item.url) + '</div>' +
        '<div class="flex flex-wrap items-center gap-3 mb-3">' +
          '<div class="text-xs text-slate-500">Outbound: <span class="font-semibold text-slate-700">' + _formatNumber(item.outboundLinks) + '</span></div>' +
          sitemapBadge +
        '</div>' +
        '<div class="text-sm text-slate-600 leading-relaxed">' + _esc(item.recommendation || _defaultOrphanRecommendation(item)) + '</div>' +
      '</div>';
    }).join('');

    container.innerHTML =
      '<div class="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 mb-6">' +
      ...
      '</div>' +
      /* Desktop: table */
      '<div class="report-table-wrap hidden sm:block">' +
      ...
      '<div class="sm:hidden space-y-3">' + mobileCards + '</div>';
  }
  ...
  function renderHubSpokeClusters(data) {
    ...
    var cards = clusters.map(function (cluster) {
      ...
      if (visibleSpokes.length) {
        spokeHtml = visibleSpokes.map(function (spoke) {
          return '<span class="inline-flex items-center rounded-full bg-slate-100 px-2 sm:px-3 py-1 text-xs font-medium text-slate-700 break-all max-w-full" title="' + _esc(spoke) + '">' +
            _esc(_shortUrl(spoke, 40)) +
          '</span>';
        }).join('');
        if (remaining > 0) {
          spokeHtml += '<span class="inline-flex items-center rounded-full bg-blue-50 px-2 sm:px-3 py-1 text-xs font-medium text-blue-700">+' + _formatNumber(remaining) + ' more</span>';
        }
      } else {
        spokeHtml = '<div class="text-sm text-slate-500">Spoke count supplied without individual spoke URLs.</div>';
      }

      return '<div class="bg-white border border-slate-200 rounded-xl p-4 sm:p-6 no-break min-w-0">' +
        '<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">' +
          '<div class="min-w-0 flex-1">' +
            '<div class="text-xs uppercase tracking-[0.16em] text-slate-400">Hub URL</div>' +
            '<div class="mt-1 text-sm sm:text-base font-semibold text-slate-800 break-all" title="' + _esc(cluster.hubUrl) + '">' + _esc(_shortUrl(cluster.hubUrl, 64)) + '</div>' +
          '</div>' +
          '<div class="text-left sm:text-right flex-shrink-0">' +
            '<div class="text-2xl sm:text-3xl font-extrabold text-blue-600 leading-none">' + _formatNumber(spokeCount) + '</div>' +
            '<div class="text-xs uppercase tracking-[0.14em] text-slate-400 mt-1">Spokes</div>' +
          '</div>' +
        '</div>' +
        '<div class="mt-4 sm:mt-5 h-3 rounded-full bg-slate-100 overflow-hidden">' +
    ...
    container.innerHTML =
      '<div class="chart-container mb-6 sm:mb-8 no-break">' +
        '<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">' +
          ...
          '</div>' +
        '</div>' +
        '<div style="height:320px;max-width:100%;overflow:hidden"><canvas id="hub-spoke-chart"></canvas></div>' +
      '</div>' +
      '<div class="grid lg:grid-cols-2 gap-4 sm:gap-6">' + cards + '</div>';

    _trackChart((window.TPPC.charts || {}).createBarChart && window.TPPC.charts.createBarChart('hub-spoke-chart', {
      horizontal: true,
      labels: clusters.map(function (cluster) { return _shortUrl(cluster.hubUrl, 32); }),
      datasets: [{
        label: 'Spokes',
  ...
  function renderLinkDepth(data) {
    ...
    var html = '';
    html += '<div class="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">';
    ...
    if (hasDistribution) {
      html += '<div class="chart-container no-break">' +
        '<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">' +
          ...
          '</div>' +
          '<div class="text-sm text-slate-500">Homepage: <span class="font-medium text-slate-700 break-all">' + _esc(_displayText(depth.homepage)) + '</span></div>' +
        '</div>' +
        '<div style="height:320px;max-width:100%;overflow:hidden"><canvas id="link-depth-chart"></canvas></div>' +
      '</div>';
    ...
    if (depth.unreachable.length) {
      html += '<div class="bg-white border border-slate-200 rounded-xl p-4 sm:p-6 mt-6">' +
        '<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">' +
          '<div>' +
        ...
        '<div class="mt-4 flex flex-wrap gap-2">' +
          depth.unreachable.slice(0, 16).map(function (url) {
            return '<span class="inline-flex items-center rounded-full bg-amber-50 px-2 sm:px-3 py-1 text-xs font-medium text-amber-800 break-all max-w-full" title="' + _esc(url) + '">' +
              _esc(_shortUrl(url, 44)) +
            '</span>';
          }).join('') +
          (depth.unreachable.length > 16
            ? '<span class="inline-flex items-center rounded-full bg-slate-100 px-2 sm:px-3 py-1 text-xs font-medium text-slate-700">+' + _formatNumber(depth.unreachable.length - 16) + ' more</span>'
            : '') +
        '</div>' +
      '</div>';
    } else if (hasDepthData) {
  ...
  function renderBacklinkProfile(data) {
    ...
    if (metrics.domain || metrics.source) {
      html += '<div class="grid sm:grid-cols-2 gap-3 sm:gap-4 mt-6 sm:mt-8">';
    ...
    if (competitors.length) {
      ...
      html += '<div class="chart-container mt-6 sm:mt-8 no-break">' +
        '<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">' +
          '<div>' +
          ...
          '</div>' +
        '</div>' +
        '<div style="height:320px;max-width:100%;overflow:hidden"><canvas id="backlink-compare-chart"></canvas></div>' +
      '</div>';

      window.setTimeout(function () {
        if ((window.TPPC.charts || {}).createBarChart) {
          _trackChart(window.TPPC.charts.createBarChart('backlink-compare-chart', {
            labels: compareSet.map(function (item) { return _shortUrl(item.domain, 24); }),
            datasets: [{
    ...
    if (topBacklinks.length) {
      /* Desktop table */
      html += '<div class="report-table-wrap mt-6 sm:mt-8 hidden sm:block">' +
        '<table class="report-table">' +
          '<colgroup>' +
            '<col style="width:25%">' +
            '<col style="width:25%">' +
          ...
          '<tbody>' +
            topBacklinks.map(function (link) {
              return '<tr class="no-break">' +
                '<td><div class="url-cell font-medium text-slate-800" title="' + _esc(link.sourceUrl) + '">' + _esc(_shortUrl(link.sourceUrl, 48)) + '</div></td>' +
                '<td><div class="url-cell text-slate-600" title="' + _esc(link.targetUrl) + '">' + _esc(_shortUrl(link.targetUrl, 48)) + '</div></td>' +
      ...
      /* Mobile cards */
      html += '<div class="sm:hidden mt-6 space-y-3">' +
        topBacklinks.map(function (link) {
          var followBadge = link.isDofollow == null
            ? '<span class="severity-badge info">Unknown</span>'
            : link.isDofollow
              ? '<span class="severity-badge low">Dofollow</span>'
          ...
          return '<div class="bg-white border border-slate-200 rounded-xl p-4 no-break">' +
            '<div class="text-xs uppercase tracking-widest text-slate-400 mb-1">Source</div>' +
            '<div class="text-sm font-medium text-slate-800 break-all mb-2" title="' + _esc(link.sourceUrl) + '">' + _esc(link.sourceUrl) + '</div>' +
            '<div class="text-xs uppercase tracking-widest text-slate-400 mb-1">Target</div>' +
            '<div class="text-sm text-slate-600 break-all mb-3" title="' + _esc(link.targetUrl) + '">' + _esc(link.targetUrl) + '</div>' +
            '<div class="flex flex-wrap items-center gap-3 text-xs text-slate-500">' +
              '<span>Anchor: <span class="font-medium text-slate-700">' + _esc(link.anchorText || 'None') + '</span></span>' +
              '<span>DR: <span class="font-semibold text-slate-700">' + _displayText(link.domainRating) + '</span></span>' +
              followBadge +
              '<span>' + _esc(_formatDate(link.firstSeen)) + '</span>' +
            '</div>' +
          '</div>';
        }).join('') +
      '</div>';
  ...
  function _detailCard(label, value, detail) {
    return '<div class="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 no-break" style="min-width:0;overflow:hidden">' +
      '<div class="text-xs uppercase tracking-[0.16em] text-slate-400">' + _esc(label) + '</div>' +
      '<div class="mt-2 text-xl sm:text-2xl font-bold text-slate-800" style="overflow-wrap:break-word;word-break:break-all">' + _esc(value) + '</div>' +
      '<p class="mt-2 text-sm text-slate-500 leading-relaxed">' + _esc(detail) + '</p>' +
    '</div>';
  }

  function _miniMetric(label, value) {
    return '<div class="rounded-xl bg-slate-50 border border-slate-200 px-3 sm:px-4 py-3">' +
      '<div class="text-xs uppercase tracking-[0.16em] text-slate-400">' + _esc(label) + '</div>' +
      '<div class="mt-1 text-base sm:text-lg font-bold text-slate-800">' + _esc(value) + '</div>' +
    '</div>';
  }
  ...
  function _listCard(title, items, emptyText) {
    var content = items.length
      ? '<ul class="mt-3 space-y-2">' + items.slice(0, 6).map(function (item) {
          return '<li class="text-sm text-slate-600 leading-relaxed flex gap-2"><span class="text-blue-500 font-bold flex-shrink-0">&#8226;</span><span class="min-w-0 break-words">' + _esc(item) + '</span></li>';
        }).join('') + '</ul>'
      : '<p class="mt-3 text-sm text-slate-500">' + _esc(emptyText) + '</p>';

    return '<div class="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 no-break min-w-0">' +
