(function () {
  ...
  function renderTextList(items, emptyLabel, limit) {
    ...
    var expandId = 'expand-' + Math.random().toString(36).substr(2, 9);
    ...
    if (values.length > maxItems) {
      ...
      values.slice(maxItems).forEach(function (item) {
        html += '<div class="text-sm text-slate-700">' + esc(item) + '</div>';
      });
      html += '</div>';
      html += '<a href="#" class="text-xs text-blue-500 hover:text-blue-700 cursor-pointer" onclick="'
        + 'var el=document.getElementById(\'' + expandId + '-hidden\');'
        + 'el.style.display=el.style.display===\'none\'?\'block\':\'none\';'
        + 'this.textContent=el.style.display===\'none\'?\'+' + esc(values.length - maxItems) + ' more\':\'Show less\';'
        + 'return false;">+' + esc(values.length - maxItems) + ' more</a>';
    }
    html += '</div>';
    return html;
  }
